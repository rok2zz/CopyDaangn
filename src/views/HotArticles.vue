...
export default class Hot_Articles extends Vue {
	...
	mounted() {
		...
		this.randomCheck = [0, 1, 2, 3, 4, 5, 6, 7]
		this.$store.commit('setMode', 'light')
	}

	// randomItem(item: number): any {
	// 	for (var i = 0; i < this.randomCheck.length; i++) {
	// 		var randomNumber = Math.floor(Math.random() * 7)

	// 		if (randomNumber == this.randomCheck[i]) {
	// 			this.randomOutput == this.products[i]
	// 			this.randomCheck[i] == null
	// 			break
	// 		}
	// 	}
